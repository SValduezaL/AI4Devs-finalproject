...
El endpoint `GET /api/admin/orders` SHALL aceptar los query params opcionales `q`, `status`, `mode`, `from` y `to`. Todos son opcionales; si no se proporcionan, el comportamiento es equivalente al endpoint sin filtros (comportamiento anterior de T01).
